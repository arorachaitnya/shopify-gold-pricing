...
async function getGoldRateINR() {
  ...
  if (!data.price_gram_24k) {
    throw new Error("GoldAPI did not return price_gram_24k");
  }
...
function calculatePrice(goldRateINR24k, metafields) {
  ...
  const weight =
    parseFloat(getMeta(metafields, "weight")) || 0;

  const purity =
    parseFloat(getMeta(metafields, "purity")) || 22;

  const makingValue =
    parseFloat(getMeta(metafields, "making_value")) || 0;

  const makingType =
    (getMeta(metafields, "making_type") || "percentage")
      .toLowerCase()
      .trim();

  const stoneCost =
    parseFloat(getMeta(metafields, "stone_cost")) || 0;

  const gst =
    parseFloat(getMeta(metafields, "gst")) || 3;
